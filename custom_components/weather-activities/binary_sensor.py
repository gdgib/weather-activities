...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback):
    ...
    async_add_entities([WeatherActivitiesDaySensor(entry=entry, coordinator=coordinator, device_info=device_info, day=day) for day in range(0,forecast_days+1)])

class WeatherActivitiesSensor(CoordinatorEntity, BinarySensorEntity):
    """Implementation of binary sensor."""

    def __init__(self, entry: ConfigEntry, coordinator: WeatherActivitiesDataCoordinator, device_info: DeviceInfo) -> None:
...
class WeatherActivitiesDaySensor(WeatherActivitiesSensor):
    ...
    def __init__(self, entry: ConfigEntry, coordinator: WeatherActivitiesDataCoordinator, device_info: DeviceInfo, day: int) -> None:
        """Initialize the binary sensor."""
        self._day = day
        super().__init__(entry=entry, coordinator=coordinator, device_info=device_info)
